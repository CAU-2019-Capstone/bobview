...
export default {
  ...
  methods: {
    ...
    initialize () {
      ...
      this.axios
        .get('http://13.124.90.6:8080/api/userorder/1/?username='+this.$store.getters.GetUserdata['username']+'&is_active=true')
        .then((result)=> {
          console.log(result.data)
    ...
    save(id) {
      let currentObj = this
      if(id > 0){
        currentObj.axios.post('http://13.124.90.6:8080/api/order/change/',{
