...
export default {
  ...
  methods: {
    ...
    initialize () {
      ...
      this.axios
      .get('13.124.90.6:8000/api/userorder/0/?username='+this.$store.state.userdata['username'])
      .then((result) => {
      ...
      this.axios
        .get('http://13.124.90.6:8000/api/userorder/1/?username='+this.$store.getters.GetUserdata['username']+'&is_active=true')
        .then((result)=> {
          console.log(result.data)
    ...
    save(id) {
      let currentObj = this
      if(id > 0){
        currentObj.axios.post('http://13.124.90.6:8000/api/order/change/',{
