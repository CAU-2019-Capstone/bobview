...
public final class LeavesView extends javax.swing.JPanel implements EditorRecord {
    private static final long serialVersionUID = 3378555023807222500L;
    ...
    private boolean IsValidEndDate(Date date) {
        ...
        if (! m_jStartDate.getText().equals("")) {
            ...
            try {
                ...
                return (startdate.before(date) || startdate.compareTo(date) == 0);
                
            } catch (BasicException ex) {
            }
        }
        return (systemDate.before(date) || systemDate.compareTo(date) == 0);
    }
...
// TODO - rewrite IsValidStartDate using Apache commons or Calendar 
    ...
    private boolean IsValidStartDate(Date date) {
        ...
        if (! m_jEndDate.getText().equals("")) {
            try {
                Date enddate = (Date) Formats.TIMESTAMP.parseValue(m_jEndDate.getText());
                validEndDate = (enddate.after(date) || enddate.compareTo(date) == 0);
            } catch (BasicException ex) {
            }
        }
        return validEndDate && (systemDate.before(date) || systemDate.compareTo(date) == 0);
    }
    ...
    private void initComponents() {
        ...
        m_jEmployeeName.setEditable(false);
        m_jEmployeeName.setFont(StartPOS.getgblFont().deriveFont( 0, 12)); // NOI18N

        m_jLeaveNote.setColumns(20);
        m_jLeaveNote.setFont(StartPOS.getgblFont().deriveFont( 0, 14));
        m_jLeaveNote.setLineWrap(true);
        m_jLeaveNote.setRows(5);
        jScrollPane1.setViewportView(m_jLeaveNote);

        m_Name.setFont(StartPOS.getgblFont().deriveFont( 0, 12)); // NOI18N
        java.util.ResourceBundle bundle = java.util.ResourceBundle.getBundle("pos_messages"); // NOI18N
        m_Name.setText(bundle.getString("label.epm.employee")); // NOI18N

        m_StartDate.setFont(StartPOS.getgblFont().deriveFont( 0, 12)); // NOI18N
        m_StartDate.setText(AppLocal.getIntString("label.epm.startdate")); // NOI18N

        m_EndDate.setFont(StartPOS.getgblFont().deriveFont( 0, 12)); // NOI18N
        ...
        m_Notes.setFont(StartPOS.getgblFont().deriveFont( 0, 12)); // NOI18N
